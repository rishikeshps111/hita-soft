...
class StockManagementController extends Controller
{
            ...
            if($privil) {
            	...
            	if($log) {
            		if($log->user_type == 1 || $log->user_type == 2 ) {
        		        $stock = StockManagement::orderBy('date', 'DESC')->get();
        		        if(sizeof($stock) != 0) {
    ...
    public function SelectQty( Request $request) {	
		...
		if($request->ajax() && isset($request->p_id)){
			...
			if($product) {
				...
				$image_url = $product->featured_product_img 
                ? asset('images/featured_products/' . $product->featured_product_img) 
                : null;
				// if(sizeof($attr) != 0) {
				// 	$tr_data = "";
    ...
    //                 }
	   ...
	   //                 </table>'; 
				...
				// 	}
				// } else {
					$error = array('error' => 1, 'product' => $product, 'data' => $data,'featured_product_img'=>$image_url);
				// }
	...
	public function SearchInvStock(Request $request) {
        ...
        $start_date = $request->get('gj_srh_srt_date');
        if($start_date) {
            $start_date = date('Y-m-d', strtotime($start_date));
        }

        $end_date = $request->get('gj_srh_end_date');
        if($end_date) {
            $end_date = date('Y-m-d', strtotime($end_date . ' +1 day'));
        }

        $pro_code = $request->get('gj_srh_pro_code');
