        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if ($validator->fails()) {
                    ...
                    if($favicon) {
                    	$img_files = $request->file('favicon_image');
                    	$old_favicon_image = $request->get('old_favicon_image');
                    	if(isset($img_files)) {
                    	    ...
                    	    Session::flash('message', 'Please upload a favicon image'); 
                            Session::flash('alert-class', 'alert-danger');
        	                 return Redirect::back();
        				}
                        
                        if($favicon->save()) {
                        	Session::flash('message', 'Favicon Image Updated successfully !'); 
            				Session::flash('alert-class', 'alert-success');
                        	return View::make("settings.favicon_setting")->with(array('favicon'=>$favicon,'page'=>$page));
                        } else{
                        	Session::flash('message', 'Favicon Image Update Failed!'); 
            				Session::flash('alert-class', 'alert-danger');
                            return Redirect::back();
                        }
                    } else{
