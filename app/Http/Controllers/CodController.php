...
class CodController extends Controller
{
    ...
    public function edit ($id) {
        ...
        if($loged) {
            ...
            if($privil) {
                $page = "Settings";
                $cod = PaymentMethod::where('id',$id)->first();
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if($cod) {
                    ...
                    if ($validator->fails()) {
                        return redirect()->back()->withErrors($validator)->with(array('cod'=>$cod, 'page'=>$page));
                    } else {
                        $data = $request->all();

                        $cod->name    = $data['name'];               
                        $cod->note      = $data['note'];                         
                        $cod->is_enabled        = $data['is_enabled'];
                        $img_files = $request->file('icon_image');
                        $old_main_cat_image = $request->get('old_icon_image');
                        if(isset($img_files)) {
                            $file_name = $img_files->getClientOriginalName();
                            $date = date('M-Y');
                            $file_path = 'images/icons/'.$date;
                            $img_files->move($file_path, $file_name);
                            $cod->icon_image = $date.'/'.$file_name;
                        } 
                        // else if(isset($old_main_cat_image) && $old_main_cat_image != '') {
                        //     $main->main_cat_image = $old_main_cat_image;
                        // }
                        else {
                            $cod->icon_image = $old_main_cat_image;
                        }
    ...
    public function Statuscod ($id) {
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if($cod) {
                    if($cod->is_enabled == 1) {
                         $activeCount = PaymentMethod::where('is_enabled', 1)
                        ->where('id', '!=', $cod->id)
                        ->count();

                    if ($activeCount < 1) {
                        Session::flash('message', 'Need at least ONE active payment method.');
                        Session::flash('alert-class', 'alert-danger');
                        return redirect()->route('manage_cod');
                    }
                        $cod->is_enabled        = 0;
                        $msg = "Payment Method Blocked Successfully";
                    } else {
                        $cod->is_enabled        = 1;
                        $msg = "Payment Method Unblocked Successfully";
                    }
                    ...
                    if($cod->save()) {
                        ...
                        Session::flash('message', 'Payment Method Failed Block or Unblock!'); 
                        Session::flash('alert-class', 'alert-danger');
                        return redirect()->route('manage_cod');
                    }
                } else{
            ...
            if($privil) {
                if($request->ajax() && isset($request->ids)){
                    ...
                    if(sizeof($ids) != 0) {
                        ...
                        foreach ($ids as $key => $value) {
                            $cod = PaymentMethod::where('id',$value)->first();
                            if($cod){
                                 if ($cod->is_enabled == 0) {
        					        Session::flash('message', 'Payment Method Already Blocked'); 
            						Session::flash('alert-class', 'alert-danger');
            						$error = 0;
                                } else {
                                $cod->is_enabled = 0;
                                $cod->save();
                                Session::flash('message', 'Payment Method Blocked Successfully!'); 
                                Session::flash('alert-class', 'alert-success');
                                $error = 0;
                                }
                            }   else {
                                Session::flash('message', 'Payment Method Blocked Failed!'); 
                                Session::flash('alert-class', 'alert-danger');
                            }           
                        }
                    } else {
                ...
                if($request->ajax() && isset($request->ids)){
                    ...
                    if(sizeof($ids) != 0) {
                        foreach ($ids as $key => $value) {
                            ...
                            if($cod){
                                 if ($cod->is_enabled == 1) {
        					        Session::flash('message', 'Payment Method Already Unblocked'); 
            						Session::flash('alert-class', 'alert-danger');
            						$error = 0;
                                } else {
                                $cod->is_enabled = 1;
                                $cod->save();
                                Session::flash('message', 'Payment Method Unblocked Successfully!'); 
                                Session::flash('alert-class', 'alert-success');
                                $error = 0;
                                }
                            }   else {
                                Session::flash('message', 'Payment Method Unblocked Failed!'); 
                                Session::flash('alert-class', 'alert-danger');
                            }           
                        }
                    } else {
