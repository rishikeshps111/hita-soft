            ...
            if($privil) {
                ...
                if ($validator->fails()) {
                    ...
                    if($sub_cats) {
                        ...
                        $img_files = $request->file('sub_cat_image');
                        if(isset($img_files)) {
            ...
            if($privil) {
                ...
                if($sub_cats) {
        	        ...
        	        if ($validator->fails()) {
                        ...
                        $img_files = $request->file('sub_cat_image');
                        $old_sub_cat_image = $request->get('old_sub_cat_image');
                        if(isset($img_files)) {
                            $file_name = $img_files->getClientOriginalName();
                            $date = date('M-Y');
                            // $file_path = '../public/images/sub_cat_image/'.$date;
        ...
        if($loged) {
            ...
            if($privil) {
        		if($request->ajax() && isset($request->id)){
        			$id = $request->id;
        			if($id != 0) {
        				$cat_bans = SubCategoryManagementSettings::where('sub_cat_id',$id)->first();
        				if($cat_bans){
        					if($cat_bans->delete()) {
        		...
        		if($request->ajax() && isset($request->ids)){
        			...
        			if(sizeof($ids) != 0) {
        				foreach ($ids as $key => $value) {
        					...
        					if($sub_cats){
        					     if ($sub_cats->is_block == 0) {
        					        Session::flash('message', 'Sub Category Already Blocked'); 
            						Session::flash('alert-class', 'alert-danger');
                                } else {

        						$sub_cats->is_block = 0;
        						$sub_cats->save();
        						Session::flash('message', 'Sub Category Blocked Successfully!'); 
        						Session::flash('alert-class', 'alert-success');
        						$error = 0;
                                }
        					}	else {
        						Session::flash('message', 'Sub Category Blocked Failed!'); 
        						Session::flash('alert-class', 'alert-danger');
        					}			
        				}
        			} else {
        		...
        		if($request->ajax() && isset($request->ids)){
        			...
        			if(sizeof($ids) != 0) {
        				foreach ($ids as $key => $value) {
        					...
        					if($sub_cats){
        					      if ($sub_cats->is_block == 1) {
        					        Session::flash('message', 'Sub Category Already Unblocked'); 
            						Session::flash('alert-class', 'alert-danger');
                                } else {

        						$sub_cats->is_block = 1;
        						$sub_cats->save();
        						Session::flash('message', 'Sub Category Unblocked Successfully!'); 
        						Session::flash('alert-class', 'alert-success');
        						$error = 0;
                                }
        					}	else {
        						Session::flash('message', 'Sub Category Unblocked Failed!'); 
        						Session::flash('alert-class', 'alert-danger');
        					}			
        				}
        			} else {
