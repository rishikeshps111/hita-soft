        		...
        		if($request->ajax() && isset($request->ids)){
        			...
        			if(sizeof($ids) != 0) {
        				foreach ($ids as $key => $value) {
        					$review = Review::where('id',$value)->first();
        					if($review){
        					    if ($review->is_block == 0) {
        					        Session::flash('message', 'Review Already Rejected'); 
            						Session::flash('alert-class', 'alert-danger');
                                } else {
            						$review->is_block = 0;
            						$review->save();
        		...
        		if($request->ajax() && isset($request->ids)){
        			...
        			if(sizeof($ids) != 0) {
        				foreach ($ids as $key => $value) {
        					...
        					if($review){
        					     if ($review->is_block == 1) {
        					        Session::flash('message', 'Review Already Approved'); 
            						Session::flash('alert-class', 'alert-danger');
                                } else {
            						$review->is_block = 1;
            						$review->save();
        						...
        						Session::flash('message', 'Review Approved Failed!'); 
        						Session::flash('alert-class', 'alert-danger');
        					}			
        				}
        			} else {
