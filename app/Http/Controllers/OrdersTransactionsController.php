...
class OrdersTransactionsController extends Controller
{
    ...
    public function AllTransaction () {
        ...
        if($loged) {
            ...
            if($privil) {
            	...
            	if( $sess) {
                    if( $sess->user_type == 1) {
                    	$trans = OrdersTransactions::OrderBy('id', 'DESC')->get();
	...
	public function SearchTrans (Request $request) {
        $page = "Transaction";                                               
        $trans_date = $request->get('gj_srh_trans_date');
        $trans_code = $request->get('gj_srh_trans_code');
        $order_code = $request->get('gj_srh_odr_code');
        $order_status = $request->get('gj_srh_odr_sts');

        if($trans_date && $trans_code) {
        	$page = "Transaction";
            $trans = OrdersTransactions::OrderBy('id', 'DESC')->Where('trans_date', 'like', '%' . $trans_date . '%')->Where('trans_code', 'like', '%' . $trans_code . '%')->get();
            ...
            if($ids && sizeof($ids) != 0) {
            	$trans = OrdersTransactions::WhereIn('id', $ids)->OrderBy('id', 'DESC')->get();
            } else {
            	$trans = OrdersTransactions::OrderBy('id', 'DESC')->get();
            }
...
elseif($trans_date) {
 			$page = "Transaction";
        	$trans = OrdersTransactions::Where('trans_date', $trans_date)->orWhere('trans_date', 'like', '%' . $trans_date . '%')->OrderBy('id', 'DESC')->get();
        	...
        	return View::make("transaction.trans.all_transaction")->with(array('trans'=>$trans, 'page'=>$page));
 			...
 			$page = "Transaction";
        	$trans = OrdersTransactions::Where('trans_code', $trans_code)->orWhere('trans_code', 'like', '%' . $trans_code . '%')->OrderBy('id', 'DESC')->get();
