...
class LogoSettingsController extends Controller
{
    ...
    public function create () {
        ...
        if($loged) {
            ...
            if($privil) {
            	$page = "Settings";
                $logo = LogoSettings::latest()->first();
            	if($logo) {
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if ($validator->fails()) {
                    ...
                    $logo = LogoSettings::first();
                    if($logo) {
                       return view('settings.logo_setting')->withErrors($validator)->with(array('logo'=>$logo,'page'=>$page));
                    } else {
                	   return view('settings.logo_setting')->withErrors($validator)->with(array('page'=>$page));
                    }
                } else {
                    $data = $request->all();
                    $id = $request->get('id');
                    ...
                    if($logo) {
                    	$img_files =  $request->file('logo_image');
                    	$old_logo_image = $request->get('old_logo_image');
                    	if(isset($img_files)) {
                    ...
                    // 	} elseif ($old_logo_image) {
                    //         $logo->logo_image = $old_logo_image;
        				// } else if(isset($old_logo_image) && $old_logo_image != '') {
            //         		$logo->logo_image = $old_logo_image;
                    	} else {
        	               // $logo->logo_image = NULL;
        	               Session::flash('message', 'Please upload a logo image'); 
                            Session::flash('alert-class', 'alert-danger');
        	                 return Redirect::back();
        				}
                        
                        if($logo->save()) {
                        	Session::flash('message', 'Logo Image Update Successfully!'); 
            				Session::flash('alert-class', 'alert-success');
                        	return View::make("settings.logo_setting")->with(array('logo'=>$logo,'page'=>$page));
                        } else{
                        	Session::flash('message', 'Logo Image Update Failed!'); 
            				Session::flash('alert-class', 'alert-danger');
                            return Redirect::back();
                        }
                    } else{
