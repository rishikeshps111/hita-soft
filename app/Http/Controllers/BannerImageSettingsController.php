    ...
    public function store(Request $request) {
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if ($validator->fails()) {
                    return view('settings.banner_image.add_banner_image')->withErrors($validator);
                } else {
                    $data = $request->all();
                    ...
                    if($banner) {
                        ...
                        $banner->button_title   = $data['button_title'];             

                        $img_files = $request->file('banner_image');
                        if(isset($img_files)) {
                        ...
                        if($banner->save()) {
                            Session::flash('message', 'Banner Image Added Successfully!'); 
                            Session::flash('alert-class', 'alert-success');
                            return redirect()->route('manage_banner_image');
                        } else{
                            Session::flash('message', 'Banner Image Added Failed!'); 
                            Session::flash('alert-class', 'alert-danger');
                            return redirect()->route('manage_banner_image');
                        }
                    } else{
    ...
    public function edit ($id) {
        ...
        if($loged) {
            ...
            if($privil) {
                ...
                if($banner) {
                    return view("settings.banner_image.edit_banner_image")->with(array('banner'=>$banner,'page'=>$page));
                } else {
            ...
            if($privil) {
                ...
                if($banner) {
                    ...
                    if ($validator->fails()) {
                        ...
                        $img_files = $request->file('banner_image');
                        $old_banner_image = $request->get('old_banner_image');
                        if(isset($img_files)) {
                            $file_name = $img_files->getClientOriginalName();
                            $date = date('M-Y');
                            $file_path = '../public/images/banner_image/'.$date;
                        ...
                        if($banner->save()) {
                            ...
                            Session::flash('message', 'Banner Image Update Failed!'); 
                            Session::flash('alert-class', 'alert-danger');
                            return redirect()->route('manage_banner_image');
                        }               
                    }
            ...
            if($privil) {
                if($request->ajax() && isset($request->id)){
                    ...
                    if($id != 0) {
                        ...
                        if($banner){
                            if($banner->delete()) {
                                Session::flash('message', 'Banner Image Deleted Successfully!'); 
                                Session::flash('alert-class', 'alert-success');
                                $error = 0;
                            } else {
                                Session::flash('message', 'Banner Image Deleted Failed!'); 
                                Session::flash('alert-class', 'alert-danger');
                                $error = 1;
                            }
                        }   else {
            ...
            if($privil) {
                ...
                if($banner) {
                    if($banner->is_block == 1) {
                        $banner->is_block        = 0;
                        $msg = "Banner Image Blocked Successfully";
                    } else {
                        $banner->is_block        = 1;
                        $msg = "Banner Image Unblocked Successfully";
                    }
                ...
                if($request->ajax() && isset($request->ids)){
                    ...
                    if(sizeof($ids) != 0) {
                        foreach ($ids as $key => $value) {
                            $banner = BannerImageSettings::where('id',$value)->first();
                            if($banner){
                                 if ($banner->is_block == 0) {
        					        Session::flash('message', 'Banner Image Already Blocked'); 
            						Session::flash('alert-class', 'alert-danger');
            						$error = 1;
                                } else {
                                $banner->is_block = 0;
                                $banner->save();
                                Session::flash('message', 'Banner Image Blocked Successfully!'); 
                                Session::flash('alert-class', 'alert-success');
                                $error = 0;
                                }
                            }   else {
                                Session::flash('message', 'Banner Image Blocked Failed!'); 
                                Session::flash('alert-class', 'alert-danger');
                            }           
                        }
                    } else {
                ...
                if($request->ajax() && isset($request->ids)){
                    ...
                    if(sizeof($ids) != 0) {
                        foreach ($ids as $key => $value) {
                            ...
                            if($banner){
                                 if ($banner->is_block == 1) {
        					        Session::flash('message', 'Banner Image Already Unblocked'); 
            						Session::flash('alert-class', 'alert-danger');
            						$error = 1;
                                } else {
                                $banner->is_block = 1;
                                $banner->save();
                                Session::flash('message', 'Banner Image Unblocked Successfully!'); 
                                Session::flash('alert-class', 'alert-success');
                                $error = 0;
                                }
                            }   else {
                                Session::flash('message', ' Banner Image Unblocked Failed!'); 
                                Session::flash('alert-class', 'alert-danger');
                            }           
                        }
                    } else {
